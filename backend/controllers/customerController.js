...
export const getCustomerCount = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: "customers count fetched from DB",
      customerCount: totalCustomers,
    });
  } catch (error) {
    console.log("error in getCustomerCount : ", error.message);
  }
};
    ...
    switch (interval) {
      ...
      case "quarterly":
        groupBy = {
          ...
          quarter: {
            $cond: [
              { $lte: [{ $month: "$created_at_date" }, 3] },
              1,
              {
                $cond: [
                  { $lte: [{ $month: "$created_at_date" }, 6] },
                  2,
                  {
                    $cond: [
                      { $lte: [{ $month: "$created_at_date" }, 9] },
                      3,
                      4,
                    ],
                  },
